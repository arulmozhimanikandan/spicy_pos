...
class WooCommerceService {
  ...
  Future<List<dynamic>> fetchAllProductsWithVariations() async {
    ...
    while (true) {
      final url = '$baseUrl/products?consumer_key=$consumerKey&consumer_secret=$consumerSecret&per_page=$perPage&page=$page';
    ...
    for (final id in parentIds) {
      int vpage = 1;
      while (true) {
        final vurl = '$baseUrl/products/$id/variations?consumer_key=$consumerKey&consumer_secret=$consumerSecret&per_page=100&page=$vpage';
    ...
    // Filter only in-stock products and variations
    return allProducts.where((p) => (p['stock_status'] ?? '').toLowerCase() == 'instock').toList();
  }
}
